...
class Department(models.Model):
    name = models.CharField(max_length=150)  # maps to "Department"
    page_link = models.URLField(blank=True, null=True)    # maps to "Dept Page Link (BO Internal Site)"
    head_email = models.EmailField(blank=True, null=True) # maps to "Dept Head Email"
    group_email = models.EmailField(blank=True, null=True) # maps to "Dept Group Email"
    external_id = models.CharField(max_length=50, blank=True, null=True) # maps to "Id"
    parent_department = models.CharField(max_length=100, blank=True, null=True) # maps to "Parent Department"
    department_type = models.CharField(max_length=50, blank=True, null=True) # maps to "Department Type (Delivery or Support)"
...
class HiringRequisition(models.Model):
    ...
    # Foreign keys — allow null/blank so existing rows don’t break
